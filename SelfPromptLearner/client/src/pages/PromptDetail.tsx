...
export default function PromptDetail() {
  ...
  useEffect(() => {
    if (prompt) setEditedText(prompt.promptText);
  }, [prompt]);
    ...
    return (
      <div className="min-h-screen bg-background flex">
        ...
        <main className="flex-1 p-8 text-center">
          ...
          <Button variant="link" onClick={() => setLocation("/")}>Back to Dashboard</Button>
        </main>
      </div>
    );
  }
  ...
  const handleSave = () => {
    if (!id) return;
    updatePrompt(
      { id, promptText: editedText },
      {
        onSuccess: () => {
          setIsEditing(false);
          toast({ title: "Changes saved", description: "Your prompt text has been updated." });
        },
      }
    );
  };
  ...
  const scoreData = prompt.analysis?.scores ? [
    { label: "Clarity", value: prompt.analysis.scores.clarity, color: "bg-emerald-500" },
    { label: "Ambiguity", value: prompt.analysis.scores.ambiguity, color: "bg-amber-500" },
    { label: "Reproducibility", value: prompt.analysis.scores.reproducibility, color: "bg-blue-500" },
  ] : [];
                ...
                {scoreData.length > 0 ? scoreData.map((score) => (
                  <div key={score.label}>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="font-mono font-bold">{Math.round(score.value * 100)}%</span>
                    </div>
